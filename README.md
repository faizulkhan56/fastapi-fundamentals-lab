...
examples (`filter/`, `search/`) - POST `/predict/` (ML prediction endpoint)
...
Script to create and train ML models:
- Generates default model for demonstration
- Can be extended for real model training
...
#### 9. ML Prediction - POST `/predict/`
- **Location**: Expand the `POST /predict/` endpoint
- **Click**: "Try it out" button
- **Request Body**: Click "Example Value" or enter manually:
  ```json
  {
...
- **Expected Response**: `200 OK` with prediction
  ...
  ```
- **Error Cases**: 
  - If model is not loaded, returns `503 Service Unavailable`
...
**Test Cases:**
- Try with different feature values
- Try with missing required fields (should return validation error)
- Try with invalid data types (should return validation error)
- Test with edge cases (very large or very small numbers)
...
- [ ] POST `/predict/` - ML prediction endpoint (test with valid and invalid features)
...
# 🤖 9. ML API Implementation

This project includes a complete ML prediction endpoint implementation.

## 9.1 Setup and Model Generation

### Step 1: Install Dependencies

All ML dependencies are included in `requirements.txt`:
...
The model is automatically created on first startup, or you can generate it manually:

```bash
python -m app.train_model
```

This creates `models/model.pkl` - a simple Linear Regression model trained on synthetic data.
...
The model is automatically loaded at startup via the `lifespan` context manager.

## 9.2 ML Endpoint Architecture

### Model Loading Flow

```mermaid
graph TD
    Startup[Application Startup] --> Check{Model Exists?}
    Check -->|Yes| Load[Load model.pkl]
    Check -->|No| Create[Create Default Model]
    Create --> Save[Save to models/model.pkl]
    Save --> Load
    Load --> Ready[Model Ready]
    Ready --> API[API Available]
```
...
Client Request
    ↓
POST /predict/ with Features JSON
    ↓
Pydantic Validation (Features model)
    ↓
Extract feature values [feature1, feature2, feature3]
    ↓
ML Model Prediction
    ↓
...
- `load_model()` - Loads model from disk
...
async def predict(features: Features):
    # Validates input, makes prediction, returns result
```

## 9.4 Using Your Own ML Model
...
2. **Save it** using joblib:
   ```python
   import joblib
   joblib.dump(your_model, "models/model.pkl")
   ```
3. **Update Features model** in `models.py` to match your input features
4. **Update prediction logic** in `model_loader.py` if needed
5. **Restart the application** - your model will be loaded automatically

## 9.5 Production Considerations

- **Model Versioning**: Store models with version numbers
- **Model Monitoring**: Log predictions and track model performance
- **Caching**: Cache predictions for frequently requested inputs
- **Batch Predictions**: Add endpoint for batch predictions
- **Model Updates**: Implement hot-reloading or graceful model updates
